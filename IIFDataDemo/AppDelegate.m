...
@interface AppDelegate () <IIFDataSdkDelegate>
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [IIFData startSdkWithAppId:@"123" appKey:@"122" authItems:@[@(AuthItemLocation), @(AuthItemHealthy)] delegate:self];
    [self setupRoot];
    return YES;
}

- (void)setupRoot {
    self.window = [[UIWindow alloc] initWithFrame:CGRectMake(0, 0, TGScreenW, TGScreenH)];
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"IIFDataDemo"];
